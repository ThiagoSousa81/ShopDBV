...
{ //Thiago Sousa -> 926380
  ...
  global $msg;   
  $cls->conectar("epiz_33087609_Users", "sql107.epizy.com", "xxxxxxxx", "xxxxxx");
  $dados = $cls->GetData($_SESSION['ID_USER']);
  $producto = $cls->GetProd('1000005');
  //-----------------------------Estava escrevendo aqui
    ...
    if (!$producto[3] == 0) {                
        try {
            ...
            $sub = $producto[3] - 1;            
            $query = mysql_query("UPDATE `PRODUTOS` SET `QNT_PROD` = '".$sub."' WHERE `PRODUTOS`.`ID_PROD` = 1000005");        
            ///--------------------------------------------------------------------------
            //echo '<h3 style="text-align: center">Carrinho vazio!</h3>';
            $cls->InsertCart($_SESSION['ID_USER'], 1000005, 0, 'NO CARRINHO', $hour, $date);
