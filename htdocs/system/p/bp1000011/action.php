...
{ //Thiago Sousa -> 926380
  ...
  global $msg;   
  $cls->conectar("xxxxxxx", "xxxxxxxxxx", "xxxxxx", "xxxxxxx");
  ...
  //-----------------------------Estava escrevendo aqui
    ...
    if (!$producto[3] == 0) {                
        try {
            ...
            $sub = $producto[3] - 1;            
            $query = mysql_query("UPDATE `PRODUTOS` SET `QNT_PROD` = '".$sub."' WHERE `PRODUTOS`.`ID_PROD` = 1000011");        
            ///--------------------------------------------------------------------------
            //echo '<h3 style="text-align: center">Carrinho vazio!</h3>';
            $cls->InsertCart($_SESSION['ID_USER'], 1000011, 0, 'NO CARRINHO', $hour, $date);
